...
fn on_mouse_click(In(point): In<Result<Vec2, ()>>, mut inp: ResMut<Inp>, mut outp: ResMut<Outp>) {
    ...
    outp.line = convex::spiral::algo(inp.points.clone());
}

fn on_spacebar_press(mut output: ResMut<Outp>, window: Single<&Window>, mut inp: ResMut<Inp>) {
    inp.points.clear();
    ...
    output.line = convex::spiral::algo(inp.points.clone());
}
