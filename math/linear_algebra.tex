...
\defn{Vector}{
  A vector can be defined as a collection of things (also called elements) where duplicates can exist and there is Order b/w any two elements.
}{
...
\defn{VectorAddition}{
  The addition of two ND vectors $A$ and $B$ ( Vector ) is a new ND vector with each element as sum of corresponding elements in $A$ and $B$.
}{
  In case of 2D with vectors say $\pvectwo{x}{y}$ and $\pvectwo{a}{b}$ the addition rule for vectors is as follows
  \[
    \pvectwo{x}{y} + \pvectwo{a}{b} = \pvectwo{x + a}{y + b}
  \]
...
\defn{VectorScaling}{
  The scaling of an ND Vector $A$ with a Number $\lambda$ produces a new ND vector with each element as product of corresponding elements in $A$ and $\lambda$.
}{
  For a number $\lambda$ and a 2D vector $\pvectwo{x}{y}$
 \[
    \lambda * \pvectwo{x}{y} = \pvectwo{\lambda * x}{\lambda * y}
  \].
